<template>
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-xl font-semibold text-gray-900">{{ totalShared ? totalShared : 0 }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-lg shadow p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-xl font-semibold text-gray-900">{{ totalSigned ? totalSigned : 0 }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-lg shadow p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-xl font-semibold text-gray-900">{{ totalPending ? totalPending : 0 }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <DataTable v-model:expandedRowGroups="expandedRowGroups" :value="customers"
    tableStyle="min-width: 50rem" expandableRowGroups rowGroupMode="subheader" groupRowsBy="user.id"
    @rowgroup-expand="onRowGroupExpand" @rowgroup-collapse="onRowGroupCollapse" sortMode="single"
    sortField="user.name" :sortOrder="1">
    <template #groupheader="slotProps">
        <span class="align-middle ml-2 font-bold leading-normal">{{ slotProps.data.user.name }}</span>
    </template>
    <Column field="document.title" header="Name"></Column>
    <Column field="pdf_path" header="Pdf path"></Column>
    <Column field="document.file_type" header="File type"></Column>
    ...
    <Column header="Actions" style="width: 10%">
        <template #body="{ data }">
            <div class="flex gap-2">
                <Button v-if="data.status == 'Signed'" icon="pi pi-download" class="p-button-rounded p-button-success p-button-sm"
                    @click="downloadDocument(data)" tooltip="Download" />
                <Button v-if="data.status == 'Pending'" icon="pi pi-bell" class="p-button-rounded p-button-warning p-button-sm"
                    @click="remindUser(data)" tooltip="Remind" />
            </div>
        </template>
    </Column>
    <template #groupfooter="slotProps">
        <div class="flex justify-end font-bold w-full">Total Customers: {{
            calculateCustomerTotal(slotProps.data.user.name) }}</div>
    </template>
</DataTable>
                <Toast />
            </div>
